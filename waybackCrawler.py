...
Aufruf:
    python waybackCrawler.py --years 2015-2025
...
def fetch_cdx_index(
    ...
    werden können (siehe ``<date_logic>`` / Duplikat-Beispiel in der Spec).

    Caching: file-basiert unter ``.cache/cdx/<slug>__<years>.json.gz`` mit
    7-Tage-TTL. SQLite (``requests-cache``) ist für CDX ungeeignet, weil
    CDX-Responses für stark archivierte Domains GB-groß werden können.
    ...
    Returns:
        Liste von ``CDXRow``-Instanzen. Leer, wenn kein Match.
    """
    ...
    def _do_request() -> requests.Response:
        ...
        # stream=True, damit der Socket-Read nicht alles in einem Rutsch in
        # den Memory zwingt (relevant bei GB-großen CDX-Antworten).
        r = session.get(
            CDX_ENDPOINT, params=params, timeout=(30, 300), stream=True
        )
    ...
    try:
        ...
        logger.error("CDX-Request für {} fehlgeschlagen: {}", target.raw, exc)
        raise

    if resp.status_code != 200:
        logger.error(
            "CDX-Request für {} lieferte Status {}", target.raw, resp.status_code
        )
        return []
    ...
    try:
        ...
        logger.error("CDX-Response für {} war kein JSON", target.raw)
        return []
    ...
    for row in rows:
        ...
        try:
            ...
            continue
    logger.info("CDX für {}: {} Rohzeilen", target.raw, len(out))
    if out:
        _write_cdx_file_cache(cache_path, out)
    return out
...
def parse_args(argv: Optional[list[str]] = None) -> Config:
    ...
    parser.add_argument(
        "--input", type=Path, default=Path("root_domains_AIOnly.xlsx")
    )
