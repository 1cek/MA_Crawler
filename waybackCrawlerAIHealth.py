...
HEALTH_KEYWORDS = {
    ...
    "diagnosis": re.compile(
        r"\b(diagnos\w*|radiolog\w*|röntgen\w*)\b", re.IGNORECASE
    ),
    "care": re.compile(
        r"\b(pflege\w*|nurs\w*|versorgung\w*)\b", re.IGNORECASE
    ),
    "treatment": re.compile(
        r"\b(therap\w*|behandlung\w*|treatment\w*)\b", re.IGNORECASE
    ),
...
def parse_args(argv: Optional[list[str]] = None) -> Config:
    ...
    parser.add_argument(
        "--input", type=Path, default=Path("root_domains_AIHealthcare_Partials.xlsx")
    )
